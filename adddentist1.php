<?php
    ...
    if(isset($_POST['add_dentist']))
    {
        ...
        $noerr = 0;
        if($mobile_no < 100000000 || $mobile_no > 10000000000)
		{
            $noerr = 1;
		...
		if($noerr == 0)
		{
            ...
            $query = "INSERT INTO dentist(user_username, dentist_name, dentist_phone, dentist_age, dentist_sex, dentist_specialization, clinic_location) VALUES ('$uname', '$name', '$mobile_no', '$age', '$s', '$type', '$clinic')"; 
            //echo $query;
    ...
    $query1 = "SELECT user.username FROM user WHERE role='dentist' AND username not in(select user.username from user, dentist where user.username=dentist.user_username)";
    $res1 = mysqli_query($db, $query1);
    $query2 = "SELECT clinic_location FROM clinic";
